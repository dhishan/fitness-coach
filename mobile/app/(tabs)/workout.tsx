...
import { ActiveSetTray } from '../../src/components/ActiveSetTray'
...
import { formatWeight, kgToDisplay, useWeightUnit } from '../../src/store/units'
...
function EntryCard({
  ...
  const addSet = () => {
    const last = entry.sets[entry.sets.length - 1] ?? (entry.tracking === 'time' ? { weight: 0, reps: 0, duration_s: 0 } : { weight: 0, reps: 0 })
      ...
      <View style={s.finishOverlay}>
        <View style={[card, s.finishCard]}>
          ...
          <Text style={s.finishVolume}>
            Total volume: <Text style={s.finishVolumeBold}>{Math.round(kgToDisplay(data.total_volume, unit)).toLocaleString()} {unit}</Text>
          </Text>
          {data.prs.length > 0 && (
              ...
              {data.prs.map((pr) => (
                <Text key={pr.exercise_id} style={s.prLine}>
                  {pr.duration_s != null
                    ...
                    : `New PR: ${pr.exercise_name} ${formatWeight(kgToDisplay(pr.weight ?? 0, unit))} ${unit} (previous ${formatWeight(kgToDisplay(pr.previous_best ?? 0, unit))} ${unit})`}
                </Text>
              ))}
            </View>
          )}
